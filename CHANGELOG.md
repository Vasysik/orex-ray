...
- Сохранение исходного URL подписки, ручное обновление серверов и отдельное лёгкое обновление metadata через `HEAD`.
...
- Балансировщики могут состоять из одного сервера или динамически включать целую папку/подписку, включая безымянную папку «Серверы».
- Для активного балансировщика интерфейс показывает последний реально использованный сервер по уже собираемой runtime-статистике, без отдельного фонового polling.
...
- Сведения подписки и составные строки с `·` адаптированы к узким окнам без висящего разделителя в конце строки.
- Папки в редакторе балансировщика по умолчанию открываются свёрнутыми.
- Обводки action-кнопок используют тот же цвет, что разделители интерфейса.

### Исправлено

- Стартовое окно Windows восстанавливает геометрию, но при обычном запуске поднимается перед другими окнами вместо появления за ними.
- В карточке подписки кликабельна вся плашка, а не только верхняя строка.
- В quick info увеличена полезная ширина только для значений `Протокол` / `Транспорт` / `Задержка`, без сдвига их подписей и самого quick switch.
- В мобильном блоке строка `Трафик` сохраняет своё положение, а значения центрируются, когда для этого достаточно места, с fallback на правое выравнивание.

## [0.6.7+10] — 2026-09-05

- Android VPN вынесен в отдельный `:vpn` process и продолжает работу независимо от Flutter UI.
- Stats/ping выполняются только при наличии реального consumer; интервалы UI-статистики, notification-статистики и ping разделены и применяются runtime там, где это безопасно.
...
- Быстрый выбор маршрута с главной и смена активного профиля работают через безопасную последовательность `stop → select → start`.
- Исправлены гонки Android `STOP → START` при быстрой смене профиля и отмена зависшего `connecting` повторным нажатием.
- Health-state больше не считается успешным до подтверждённого route ping; timeout/unavailable показываются как `Нет ответа` / `Ошибка соединения`.
- Добавлены страновые флаги выхода и отдельный Cloudflare/WARP badge.
- Унифицированы debug/release PowerShell build tools и ADB-сбор диагностики.
- Исправлены lifecycle/dispose races в `TunnelController` и фоновых latency probes, а также связанные widget-test regressions.
