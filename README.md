...
OrexRay **0.6.5+7** находится в стадии private beta / dogfood, но текущий фокус уже не на добавлении базовых режимов, а на надёжности long-running VPN. Android VPN уже
...
привязывает к нему outbound Xray. После resume и смены Wi‑Fi/Ethernet/точки доступа
соединение пересобирается с новым интерфейсом. Это важнее надписи Windows
«Нет доступа к Интернету» на виртуальном TUN-интерфейсе: значок сети сам по себе не
является проверкой маршрутизации.


## Надёжность long-running подключения

В 0.6.5+7 добавлен recovery-контур для сценария «забыл, что VPN существует»:

- Windows получает события resume и изменения IP-интерфейсов от ОС;
- активный TUN после смены физической сети пересобирается с новой outbound-привязкой;
...
- физический outbound-интерфейс Windows TUN;
- watchdog/restart budget, последнюю ошибку и exit code;
- последние 100 строк runtime-журнала.
...
Версия **0.6.5+7** сфокусирована на надёжности и наблюдаемости:
