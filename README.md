...
- добавлять стандартные HTTP(S)-подписки, обновлять их вручную и автоматически;
- читать из подписки raw-список `vless://`/`vmess://`/`trojan://`/`ss://`,
  legacy Base64-список или JSON Xray;
- читать метаданные подписки `profile-title`, `subscription-userinfo`,
  `profile-update-interval`, `support-url`, `profile-web-page-url` и `announce`
  из HTTP-заголовков, `#`- или `//`-строк тела; строковые metadata
  понимаются и в plain text, и в `base64:` UTF-8, а значения из тела имеют
  приоритет над одноимёнными HTTP-заголовками;
...
- держать массовое выделение серверов и подписок раздельным: «Выбрать всё» действует только в текущем разделе;
- объединять ручные профили в пользовательские группы;
- раскрывать каждую подписку прямо в разделе «Подписки» как группу со своими серверами — без дублирующей папки в разделе «Серверы»;
- менять порядок серверов drag-and-drop внутри текущей ручной группы;
- выделять группу долгим нажатием вместе со всем её содержимым, а затем перетаскивать группу как единый блок;
- расформировывать ручную группу без удаления профилей; для удаления всей группы с содержимым достаточно выделить её и использовать обычное массовое удаление;
- быстро выбирать маршрут с главной страницы через сворачиваемые группы; группа текущего профиля раскрывается сразу, а если есть только обычные несгруппированные серверы — быстрый выбор остаётся плоским без лишней строки «Серверы»;
...
и профили. Карточка подписки показывает стандартный `subscription-userinfo` как
использованный/общий/оставшийся трафик и срок действия, а `support-url` и
`profile-web-page-url` — как действия «Поддержка» и «Кабинет». Между полными
обновлениями OrexRay не скачивает весь список серверов только ради квоты: при
запуске и возврате в foreground при необходимости выполняется лёгкий HTTP HEAD.
Интервал проверки метаданных настраивается отдельно от интервала обновления
серверов и может быть полностью отключён. Если провайдер HEAD не поддерживает,
метаданные просто обновятся при следующем обычном GET. Ошибка такой проверки не
затрагивает рабочие профили.

Автообновление серверов не держит постоянный фоновый таймер: при запуске
OrexRay и при возвращении приложения в foreground проверяется только срок
последнего успешного обновления. Пользователь задаёт минимальный интервал в
«Фоновая работа», а если провайдер прислал `profile-update-interval`, OrexRay не
обновляет подписку чаще этой рекомендации. Неудачная автоматическая попытка не
уничтожает старые серверы и не блокирует запуск приложения. При первом запросе
OrexRay использует собственный User-Agent; если панель вернула неподдерживаемый
ответ, запрос один раз повторяется с compatibility User-Agent, поскольку некоторые
панели выбирают формат подписки по User-Agent.

Служебные subscription-ноды, которые указывают на loopback (`localhost`,
`127.0.0.0/8`, `::1`), не попадают в список серверов: их названия сохраняются
как raw/legacy-информационные строки провайдера и показываются в карточке
подписки через разделитель `·`. OrexRay не пытается извлечь из произвольных
remark вроде «80/80GB» числовую квоту: полноценная шкала трафика и срок строятся
только из стандартного `subscription-userinfo`.
Кнопки «Поддержка» и «Кабинет» создаются только из явных `support-url` и
`profile-web-page-url`; OrexRay не угадывает ссылки по именам или remark нод.

Vendor-specific encrypted-ссылки (`happ://crypto...`) и провайдерские подписки,
требующие специальный `X-HWID` или нестандартную идентификацию клиента, пока
не считаются поддержанными стандартными подписками.
...
Достаточно одного сервера: это полезно как заготовка, для fallback-сценариев и
для динамических папок. Можно выбрать отдельные серверы, целые пользовательские
группы и группы подписок, а также смешивать оба способа. Выбор устроен так же,
как быстрый переключатель на главной: группы раскрываются, а внутри видны их
серверы и индивидуальные checkbox'ы.

Папка в балансировщике остаётся **динамическим источником**: если в неё позже
добавится профиль или подписка обновит набор серверов, состав балансировщика
изменится автоматически. В Xray-конфиг передаётся уже разрешённый набор
outbound'ов; отдельной искусственной иерархии тегов OrexRay не создаёт.
...
Для Xray Observatory используется общий URL из «Сеть → Проверка задержки».
В самом балансировщике остаётся только интервал наблюдения; отдельный probe URL
там больше не настраивается.
...
**0.6.7+10** — опубликованная стабильная версия. **0.7.0+11** — текущая
разрабатываемая версия, с которой OrexRay становится полностью open source и
получает больше пользовательских возможностей поверх уже работающего Xray-клиента.

В 0.7.0 уже ведётся работа над:

- HTTP(S)-подписками с ручным и событийным автообновлением серверов и отдельным лёгким обновлением метаданных через HEAD;
- квотой/сроком/ссылками подписки из стандартных metadata без угадывания по названиям служебных нод;
- экспортом исходных URL подписок и их серверов как JSON Xray;
- подписками как самостоятельными раскрываемыми группами, пользовательскими группами профилей, раздельным групповым выделением/drag-and-drop и группированным быстрым выбором;
- адаптивной панелью массовых действий, которая сохраняет одну строку и убирает подписи у кнопок при нехватке ширины;
- удобным управлением десятками и сотнями серверов;
- переключением активного профиля без блокировки интерфейса;
- единым поведением статусов маршрута на Android и Windows;
- применением runtime-настроек статистики и ping без переподключения там, где это
  поддерживается безопасно;
- обновлением OrexRay из самого приложения по схеме, близкой к Orex Messenger.

Автообновление подписок не возвращает постоянный polling: проверка выполняется
при запуске/возврате приложения и только если заданный интервал уже истёк. Частые
stats/ping циклы по-прежнему управляются отдельными настройками.
