...
- добавлять стандартные HTTP(S)-подписки Happ/V2Ray, обновлять их вручную и автоматически;
...
- читать метаданные подписки `profile-title`, `subscription-userinfo`,
  ...
  из HTTP-заголовков или `#`-строк тела;
...
- быстро выбирать маршрут с главной страницы через сворачиваемые группы; группа текущего профиля раскрывается сразу;
...
запуске и возврате в foreground не чаще раза в 10 минут выполняется лёгкий
HTTP HEAD. Если провайдер HEAD не поддерживает, метаданные просто обновятся при
следующем обычном GET. Ошибка такой проверки не затрагивает рабочие профили.
...
ответ, запрос один раз повторяется с v2rayN-совместимым UA, поскольку некоторые
...
как legacy-информационные строки подписки. Стандартные `subscription-userinfo`
и URL-метаданные имеют приоритет, поэтому совместимые «Осталось 23 дня»/
«80/80GB»/`t.me/...` ноды не дублируют нормальные данные провайдера; `t.me`
может использоваться как fallback-ссылка поддержки.
...
- HTTP(S)-подписками с ручным и событийным автообновлением;
- подписками как самостоятельными раскрываемыми группами, пользовательскими группами профилей, групповым выделением/drag-and-drop и группированным быстрым выбором;
