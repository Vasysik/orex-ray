...
- добавлять стандартные HTTP(S)-подписки Happ/V2Ray и обновлять их вручную;
- читать из подписки raw-список `vless://`/`vmess://`/`trojan://`/`ss://`,
  legacy Base64-список или JSON Xray;
- читать метаданные подписки `profile-title`, `subscription-userinfo` и
  `profile-update-interval` из HTTP-заголовков или `#`-строк тела;
...
Подписка хранится как отдельный источник: её URL не превращается в HTTP-proxy.
При обновлении OrexRay заменяет только серверы этой подписки, сохраняя вручную
добавленные профили. Удаление подписки удаляет только принадлежащие ей серверы.
На Android URL подписок сохраняются через тот же защищённый storage channel, что
и профили. Фоновый polling подписок не запускается: обновление сейчас ручное,
чтобы не добавлять незаметную сетевую активность и расход батареи. При первом
запросе OrexRay использует собственный User-Agent; если панель вернула
неподдерживаемый ответ, запрос один раз повторяется с v2rayN-совместимым UA,
поскольку некоторые панели выбирают формат подписки по User-Agent.

Encrypted-ссылки Happ (`happ://crypto...`) и провайдерские подписки, требующие
специальный `X-HWID`/подмену User-Agent, пока не считаются поддержанными
стандартными подписками.
