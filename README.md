...
Перед состоянием «Подключено» OrexRay дополнительно проверяет лучшие публичные
IPv4/IPv6-маршруты Windows. Для dual-stack системы проверка ждёт обе семьи адресов,
чтобы не принять короткий переходный момент IPv4 как готовое подключение. Надпись
Windows «Нет доступа к Интернету» на самом виртуальном TUN-интерфейсе при этом не
подменяется искусственно: экран **Диагностика** показывает фактический маршрут.
...
- перед статусом «Подключено» проверяется захват публичных IPv4/IPv6-маршрутов;
...
- физический outbound-интерфейс и фактический IPv4/IPv6-маршрут Windows TUN;
