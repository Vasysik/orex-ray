...
появляется компактный ISO-код страны, а WARP отмечается отдельным значком молнии.
...
- при активном VPN менять маршрут только с главной: экран профилей блокирует
  случайное переключение, чтобы тяжёлый stop/start не маскировался обычным tap.
